...
export class Global {
  ...
  on() {
    ...
    //获取(platformPath)
    ipcMain.handle('global-platform-path-get', (event, path) => {
      return this.getPlatformPath(path);
    });
  }
    ...
    try {
      path = normalize(
        app.isPackaged
          ...
          : resolve(join('src', 'lib', 'resources', 'inside', path))
      );
    ...
    try {
      path = normalize(
        app.isPackaged
          ...
          : resolve(join('src', 'lib', 'resources', 'platform', process.platform, path))
      );
      accessSync(path, constants.R_OK);
      return path;
    } catch (e) {
      logError(`[getPlatformPath ${path}]`, e);
      return
    }
  }
    ...
    try {
      path = normalize(
        app.isPackaged
          ...
          : resolve(join('src', 'lib', 'resources', 'extern', path))
      );
      accessSync(path, constants.R_OK);
      return path;
    } catch (e) {
      logError(`[getExternPath ${path}]`, e);
      return
    }
  }
    ...
    try {
      path = normalize(
        app.isPackaged
          ...
          : resolve(join('src', 'lib', 'resources', 'root', path))
      );
      accessSync(path, constants.R_OK);
      return path;
    } catch (e) {
